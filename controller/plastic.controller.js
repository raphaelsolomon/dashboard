...
exports.plasticIndex = async(req, res, next) => {
    ...
    const totalRetrieveProduct = await sequelize.query('SELECT count(DISTINCT(`product`)) AS `count`, product  FROM `plastics`');

    const totalRetrieveLocation = await sequelize.query('SELECT count(DISTINCT(`retrieved_from`)) AS `count`, retrieved_from FROM plastics');

    //DAYS WITH THE HIGHEST RETRIEVAL
    const daysWithHighestRetrieval = await sequelize.query('SELECT count(DISTINCT(`date`)) AS `count`, date FROM plastics');

    //count of retrieved pet plastics by brands daily
    const retrievalBrandDaily = await req.user.getPlastics({
        where: { date: date.format(new Date(Date.now()), 'MM/DD/YYYY') },
        attributes: [
            'manufacturer', [sequelize.fn('count', sequelize.col('manufacturer')), 'count'],
        ],
        group: ['plastics.product'],
        raw: true,
        limit: 10,
        order: sequelize.literal('count DESC')
    });
    ...
    console.log(brandWithHighestProduct)
    ...
    var data = {
        ...
        totalRetrievedProduct: totalRetrieveProduct[0][0],
        totalRetrieveLocation: totalRetrieveLocation[0][0].retrieved_from,
        daysWithHighestRetrieval: daysWithHighestRetrieval[0][0].date,
    ...
    return res.status(200).render('../plastics/index', { user: req.user, data: data })
}
