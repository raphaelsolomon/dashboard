...
exports.logisticIndex = async (req, res, next) => {
  ...
  const getTotalCustomers = await require('../chartQuery/logistic.query').getTotalCustomers(req, res);
  const getTotalOrders = await require('../chartQuery/logistic.query').getTotalOrders(req, res);
  const getTotalAmount = await require('../chartQuery/logistic.query').getTotalAmount(req, res);
  const mostDeliveredLocation = await require('../chartQuery/logistic.query').mostDeliveredLocation(req, res);

  //Amount Of Money Made Daily
  var data = {
    tableLimit: limitLogist,
    totalCustomer: getTotalCustomers,
    totalOrder: getTotalOrders,
    totalAmount: getTotalAmount,
    mostDeliveredLocation: mostDeliveredLocation
  };
...
exports.addItem = async (req, res, next) => {
  ...
  return res
    .status(200)
    .render("../logistics/add", { user: req.user, title: "Add Items", notification: notification,  isUsed: true, });
};
...
exports.getEdit = async (req, res, next) => {
  ...
  return res.status(200).render("../plastics/edit", { user: req.user, notification: notification, logistic, title: 'Edit Logistics',  isUsed: true, });
}
