...
exports.addItem = async (req, res, next) => {
  ...
  return res
    .status(200)
    .render("../logistics/add", { user: req.user, title: "Add Items", n });
};
...
exports.getCharts = async (req, res, next) => {
  ...
  const moneyMadeDaily = await req.user.getLogistics({
    where: {
      amount_paid: {
        [Op.in]: [
          sequelize.literal(
            "SELECT MAX(amount_paid) FROM logistics GROUP BY dateOfOrder"
          ),
        ],
      },
    },
  ...
  const frequentPickup = await req.user.getLogistics({
    attributes: [
      "pickupaddress_area",
      [sequelize.fn("count", sequelize.col("pickupaddress_area")), "count"],
      //[sequelize.fn('sum', sequelize.col('noOfPerson')), 'noOfPerson']
    ],
  ...
  const deliveredGoods = await req.user.getLogistics({
    attributes: [
      "itemToBeDelivered",
      [sequelize.fn("count", sequelize.col("itemToBeDelivered")), "count"],
    ],
  ...
  const patronizeConsignee = await req.user.getLogistics({
    attributes: [
      "consignee",
      [sequelize.fn("count", sequelize.col("consignee")), "count"],
    ],
  ...
  const patronizeConsignor = await req.user.getLogistics({
    attributes: [
      "consignor",
      [sequelize.fn("count", sequelize.col("consignor")), "count"],
    ],
