...
exports.commodityIndex = async (req, res, next) => {
  ...
  const completedTrans = await sequelize.query(`SELECT status, COUNT(*) AS magnitude FROM commodities   re userId=${req.user.id} GROUP BY status ORDER BY magnitude DESC LIMIT 2`)
