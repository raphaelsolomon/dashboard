...
exports.index = async (req, res) => {
    ...
    const highestLocationByDate = await Plastic.findAll({
    ...
    const highestLocation = await Plastic.findAll({
        attributes: ['retrieved_from', [sequelize.fn('COUNT', sequelize.col('retrieved_from')), 'total']],
        limit: 2,
        raw: true,
        group: 'retrieved_from', order: [[sequelize.fn('COUNT', sequelize.col('retrieved_from')), 'DESC']]
    ...
    const highestBrand = await Plastic.findAll({
        attributes: ['retrieved_from', [sequelize.fn('COUNT', sequelize.col('retrieved_from')), 'total']],
        limit: 2,
        raw: true,
        group: 'retrieved_from', order: [[sequelize.fn('COUNT', sequelize.col('retrieved_from')), 'DESC']]
    ...
    return res.status(200).render('../admin/plastics/index', { users: req.user, plastic: plastic, highestLocation: highestLocation,highestBrand: highestLocationByDate: highestLocationByDate });
}
