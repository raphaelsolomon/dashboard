...
async function getToken() {
    ...
    const resp = await axios({
        method: 'post', 
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        url: `https://login.microsoftonline.com/${TENANT_ID}/oauth2/token`, data: `client_id=${credentials.clientId}&Client_secret=${credentials.secret}&grant_type=client_credentials&scope=https://graph.microsoft.com/.default&refresh_token=${refreshToken}`}).catch((err) => console.log(err))
     return resp;
  }
