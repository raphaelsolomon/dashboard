...
async function getToken() {
    ...
    let token = fetch(`https://login.microsoftonline.com/${TENANT_ID}/oauth2/v2.0/token`, {
            method: 'post',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `client_id=${credentials.clientId}&Client_secret=${credentials.secret}&grant_type=client_credentials&scope=https://graph.microsoft.com/.default&refresh_token=`
        }).then(function(response) {
            return response.json()
        }).then(json => {
            console.log(json)
