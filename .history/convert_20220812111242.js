...
async function getToken() {
    ...
    let token = fetch(`https://login.microsoftonline.com/${TENANT_ID}/oauth2/v2.0/token`, {
            ...
            body: `client_id=${credentials.clientId}&Client_secret=${credentials.secret}&grant_type=client_credentials&scope=https://graph.microsoft.com/.default&refresh_token=${refresh_token}`
        }).then(function(response) {
    ...
    const resp = await axios({method: 'post', url: `https://login.microsoftonline.com/${TENANT_ID}/oauth2/token`, data: `client_id=${credentials.clientId}&Client_secret=${credentials.secret}&grant_type=client_credentials&scope=https://graph.microsoft.com/.default&refresh_token=${refresh_token}`}).catch((err) => console.log(err))
     return resp;
  }
