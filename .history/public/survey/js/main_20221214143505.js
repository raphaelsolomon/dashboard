(function ($) {
    ...
    form.children("div").steps({
        ...
        onFinished: function (event, currentIndex) {
            ...
            var email = $("#email").val();
            var countrys = $("#countrys").find(":selected").val() === 'Other ? ';
            var phone = $("#phone").val();
            var date_of_birth = $("#date_of_birth").val();

            var gender = $('#gender').find(":selected").val();

            var emp_status = $('#emp_status').find(":selected").val() === 'Other' ? $("#em_status_other").val() : $('#emp_status').find(":selected").val();
            ...
            var osh_qualifications = $('#osh_qualifications').find(":selected").val() === 'Other' ? $('#osh_qualifications').find(":selected").val() : $("#ed_level_other").val();

            var years_of_experiences = $('#years_of_experiences').find(":selected").val();
            var osh_field_of_expertise = $('#osh_field_of_expertise').find(":selected").val() === 'Other' ? $("#osh_qua_other").val() :  $('#osh_field_of_expertise').find(":selected").val();
...
function osh_other(value) {
    if (value === 8) {
        $("#osh_qua_other").css("display", "block")
    } else {
        $("#osh_qua_other").css("display", "none")
    }
}
