...
route.post("/register", [
  ...
  check('phone_number', 'PhoneNumber is not valid').isLength({min: 11, max: 12}).normalizePhoneNumber()
  check('password', 'Password is not valid').isLength({ min: 6 }).normalizeEmail(),
], async (req, res, next) => {
...
if(!errors.isEmpty) {
  res.render("../auths/register", {errors})
}
...
route.get("/profile", isAuthenticated, async (req, res, next) => {
  ...
  return res
    .status(200)
    .render("../auths/profile", { user: user, title: "Profile", notification: notification });
});
