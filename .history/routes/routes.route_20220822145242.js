...
route.get('/recent/:id', isAuthenticated, async (req, res, next) => {
    ...
    if (req.user.trade === 'Commodity') {
        return require('../controller/commodity.controller').recentId(req, res, next);
    }
})
...
route.get('/chart', isAuthenticated, async (req, res, next) => {
    ...
    if (req.user.trade === "Commodity") {
        return require('../controller/commodity.controller').getCharts(req, res, next);
    }
})
...
route.get('/delete/:id', isAuthenticated, async (req, res, next) => {
    ...
    if (req.user.trade === 'Commodity') {
        return require('../controller/commodity.controller').deleteItem(req, res, next);
    }
})
...
route.post('/forget-password', async (req, res, next) => {
    ...
    if (user) {
        user.token = `${token}`;
        user.save();
        let sentEmail = sendLink(`http://dechdash.net/reset-password?id=${token}}`, email);
        if (sentEmail) {
...
route.post('/search', async (req, res, next) => {
    ...
    const notification = await req..getNotifications({where: {isseen: false}})
    ...
    if (req.user.trade === "Plastics") {
        return Plastic.findAll({
            ...
            return res.status(200).render('../plastics/search', { listItem: plastic, user: req.user, title: 'Search' })
        })
    }
    ...
    if (req.user.trade === "Commodity") {
        return Commodity.findAll({
            ...
            return res.status(200).render('../commodities/search', { listItem: commodity, user: req.user, title: 'Search' })
        })
    }
})
