...
route.get('/external/:id/', isAuthenticated, async (req, res) => {
    return ExternalAudit.destroy({ where: { id: req.params.id } }).then((_) => {
        return res.status(200).redirect('/admin/hse/table/general');
    });
})
