...
route.post('/general', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return GeneralInput.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/general_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/general_input', { alert: msg, input: e });
      });
    });
  } else {
...
route.post('/overview', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return IncidentOverview.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/incident_overview_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/incident_overview_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/hse_inspection', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return HSE_INSPECTION.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/hse_inpsection_report_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/hse_inpsection_report_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/contractor', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return Contractors_Mgt.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/contractor_management_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/contractor_management_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/external', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return ExternalAudit.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/external_audit_inspection_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/external_audit_inspection_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/hse_onduty', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return HSE_ONDUTY.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/hse_personnel_onduty_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/hse_personnel_onduty_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/others', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return ORTHER_HSE.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/other_hse_activities_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/other_hse_activities_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.post('/water_and_oil', isAuthenticated, async (req, res, next) => {
  if (req.body.update !== undefined) {
    return Water_And_Oil.findOne({ where: { id: req.body.id } }).then((e) => {
      return e.update(req.body).then((result) => {
        const msg = [];
        msg.push({ msg: "Record Successfully Updated", err: false });
        return res.status(200).render('../hse/edits/water_and_oil_record_input', { alert: msg, input: result });
      }).catch((err) => {
        const msg = [];
        msg.push({ msg: `Unable to updated record ${err}`, err: true });
        return res.status(200).render('../hse/inputs/water_and_oil_record_input', { alert: msg, input: e });
      });
    });
  } else {
    if (req.body.date === undefined) {
...
route.get('/contractor/:id', isAuthenticated, async (req, res, next) => {
  return Contractors_Mgt.findOne({ where: { id: req.params.id } }).then((e) => {
    return res.status(200).render('../hse/edits/contractor_management_input', { alert: [], input: e });
  });
})

route.get('/external/:id', isAuthenticated, async (req, res, next) => {
  return ExternalAudit.findOne({ where: { id: req.params.id } }).then((e) => {
    return res.status(200).render('../hse/edits/external_audit_inspection_input', { alert: [], input: e });
  });
})

route.get('/hse_onduty/:id', isAuthenticated, async (req, res, next) => {
  return HSE_ONDUTY.findOne({ where: { id: req.params.id } }).then((e) => {
    return res.status(200).render('../hse/edits/hse_personnel_onduty_input', { alert: [], input: e });
  });
})

route.get('/others/:id', isAuthenticated, async (req, res, next) => {
  return ORTHER_HSE.findOne({ where: { id: req.params.id } }).then((e) => {
    return res.status(200).render('../hse/edits/other_hse_activities_input', { alert: [], input: e });
  });
})

route.get('/water_and_oil/:id', isAuthenticated, async (req, res, next) => {
  return Water_And_Oil.findOne({ where: { id: req.params.id } }).then((e) => {
    return res.status(200).render('../hse/edits/water_and_oil_record_input', { alert: [], input: e });
  });
})
