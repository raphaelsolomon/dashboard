...
route.get("/changeRead/:id", isAuthenticated, async (req, res, next) => {
    const notification = await req.user.getNotifications({where: {id: req.params.id}});
    notification.isseen = true;
    notification.save().then((_) => {
        return res.status(200).json("../commodities/search", {
    })
  });
