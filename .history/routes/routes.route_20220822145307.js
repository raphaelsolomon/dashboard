...
route.post('/search', async (req, res, next) => {
    ...
    if (req.user.trade === "Plastics") {
        return Plastic.findAll({
            ...
            return res.status(200).render('../plastics/search', { listItem: plastic, user: req.user, notification:notification,title: 'Search' })
        })
    }
    ...
    if (req.user.trade === "Commodity") {
        return Commodity.findAll({
            ...
            return res.status(200).render('../commodities/search', { listItem: commodity, user: req.user,notification:notification, title: 'Search' })
        })
    }
})
