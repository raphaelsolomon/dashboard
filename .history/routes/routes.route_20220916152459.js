...
route.get('/newusers',  async (req, res, next)=> {
  const date = new Date();
...
route.get('/activeusers',  async (req, res, next)=> {
  const users = await User.findAll({
    where: { trade: 'Commodity' }, include: [{
        model: Commodity
    }]});
    const inActive = [users].filter((e) => e.commodities.length )
  return res.status(200).render('../admin/activeusers', {});
});

route.get('/inactiveusers',  async (req, res, next)=> {
  return res.status(200).render('../admin/inactiveusers', {});
});
