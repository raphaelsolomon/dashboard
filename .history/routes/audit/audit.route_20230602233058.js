...
route.get('/section6', isAuthenticated, async (req, res) => {
    ...
    payload = payloadAudit(req, data);
    res.status(200).render('../audit/inputs/section6', { alert: [], user: user });
});

route.post('/submission', isAuthenticated, async (req, res) => {
    const result = payload;
    result['images'].poor_house_keeping = req.body['path[poor_house_keeping]'] === undefined ?'': req.body['path[poor_house_keeping]'];
    result['images'].trailing_cable = req.body['path[trailing_cable]'] === undefined ?'': req.body['path[trailing_cable]'];
    result['images'].fire_fighting_equipment = req.body['path[fire_fighting_equipment]'] === undefined ?'': req.body['path[fire_fighting_equipment]'];
    result['images'].waste_area = req.body['path[poor_house_keeping]'] === undefined ?'': req.body['path[waste_area]'];
    result['images'].stairs_and_staircase = req.body['path[stairs_and_staircase]'] === undefined ?'': req.body['path[stairs_and_staircase]'];
    result['images'].safety_signs = req.body['path[safety_signs]'] === undefined ?'': req.body['path[safety_signs]'];
    result['images'].workstations = req.body['path[workstations]'] === undefined ?'': req.body['path[workstations]'];
    result['images'].toilets = req.body['path[toilets]'] === undefined ?'': req.body['path[toilets]'];
    result['images'].lighting = req.body['path[lighting]'] === undefined ?'': req.body['path[lighting]'];
    result['images'].muster_point = req.body['path[muster_point]'] === undefined ?'': req.body['path[muster_point]'];
    result['images'].hazard_spotted = req.body['path[hazard_spotted]'] === undefined ?'': req.body['path[hazard_spotted]'];
    result['images'].front_of_the_bank = req.body['path[front_of_the_bank]'] === undefined ?'': req.body['path[front_of_the_bank]'];
    result['images'].generator_area = req.body['path[generator_area]'] === undefined ?'': req.body['path[generator_area]'];
