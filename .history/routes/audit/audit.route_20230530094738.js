...
route.get('/table3', isAuthenticated, async (req, res) => {
    ...
    const fire_precautions = await AuditFirePrecautions;
    const access_exit = await AuditAccessExits;
    const safety_signs = await AuditSafetySigns.f;
    const stairs_staircases = await AuditStairCases;
