...
route.get('/introduction', isAuthenticated, async (req, res) => {
    ...
    const bank_audit = await AuditBankAudit.findAll({ where: { userId: req.user.id } });
    const welfare = await AuditExecutiveSummary.findAll({ where: { userId: req.user.id } });
    res.status(200).render('../audit/table/introdction', { alert: [], welfare: welfare, user: user, bank_audit: bank_audit });
});
