...
route.get('/section/table2', isAuthenticated, async (req, res) => {
    ...
    const control_substances = await 
    const electric = await 
    res.status(200).render('../audit/table/sectionTable2', { alert: [], injuries: injuries, first_aid: first_aid, first_safety: first_safety, lifting: lifting, control_substances: control_substances, electric:  });
});
