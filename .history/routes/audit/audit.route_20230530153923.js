...
route.get('/introduction', isAuthenticated, async (req, res) => {
    ...
    res.status(200).render('../audit/inputs/introduction', { alert: [], user:  });
});
...
route.get('/table1', isAuthenticated, async (req, res) => {
    ...
    res.status(200).render('../audit/table/sectionTable1', { alert: [], mgtsReviews: mgtsReviews, insurance: insurance, emgProceedure: emgProceedure, auditSafetyPolic: auditSafetyPolic, incident: incident,  wasteMgts: wasteMgts,
        training: training, safe_system: safe_system, permit_work: permit_work, communication: communication, contractorsMgts: contractorsMgts
    });
});
