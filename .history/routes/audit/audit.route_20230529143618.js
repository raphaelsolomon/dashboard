...
route.get('/section/table2', isAuthenticated, async (req, res) => {
    res.status(200).render('../audit/table/sectionTable2', { alert: [], });
});

route.get('/section/table3', isAuthenticated, async (req, res) => {
    res.status(200).render('../audit/table/sectionTable3', { alert: [], });
});
route.get('/section/table4', isAuthenticated, async (req, res) => {
    res.status(200).render('../audit/table/sectionTable4', { alert: [], });
});
route.get('/section/table5', isAuthenticated, async (req, res) => {
    const contractorsMgts = await AuditWelFare.findAll({ where: { userId: req.user.id } });
    
    res.status(200).render('../audit/table/sectionTable5', { alert: [], });
});
