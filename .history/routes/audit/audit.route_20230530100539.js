...
const { AuditTemperatures, AuditStairCases, AuditSafetySigns, AuditAccessExits, AuditFirePrecautions, AuditGangWays, AuditFloors } = require("../../model/audit/environment.model");
...
route.get('/table3', isAuthenticated, async (req, res) => {
    ...
    const floors = await AuditFloors.findAll({ where: { userId:
    res.status(200).render('../audit/table/sectionTable3', { alert: [], temperature: temperature, fire_precaution: fire_precautions, access_exit: access_exit, safety_signs: safety_signs, stairs_staircases: stairs_staircases, gangways: gangways });
});
