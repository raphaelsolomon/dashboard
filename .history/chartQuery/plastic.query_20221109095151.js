...
exports.getVolumeOfPlasticsWithTonnage = async (req) => {
    let map = {};
    const plastics = await req.user.getPlastics({ limit: 7, raw: true });
    plastics.array.forEach(element => {
        if( map[`${plastics.tonnage}${plastics.plastic_size}`] === undefined ) {
            map[`${plastics.tonnage}${plastics.plastic_size}`] = Number.parseInt(plastics.volume_of_plastics);
        }else {
            map[`${plastics.tonnage}${plastics.plastic_size}`] = map[`${plastics.tonnage}${plastics.plastic_size}`] +  Number.parseInt(plastics.volume_of_plastics);
        }
    });
