...
exports.getVolumeOfPlasticsWithTonnage = async (req) => {
    let map = {};
    const plastic = await req.user.getPlastics({ raw: true, limit: 7, order: [['volume_of_plastics', 'DESC']] });
...
exports.initialContentByVolumn = async (req) => {
    ...
    plastic.forEach(plastics => {
        if (map[plastics.initial_content] === undefined) {
            ...
            map[plastics.initial_content] = map[`${plastics.plastic_size} ${plastics.tonnage}`] + Number.parseInt(plastics.volume_of_plastics);
        }
    });
    return map;
}