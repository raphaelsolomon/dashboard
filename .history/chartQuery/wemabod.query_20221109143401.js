...
exports.streetVisitedByMonth = async (req, res) => {
    let map = {};
    const wemabods = await Wemabod.findAll({ raw: true, order: [['date', 'DESC']] });
    wemabods.forEach(wemabod => {
        if (map[wemabod.date] === undefined) {
            map[wemabod.date] = 10;
        } else {
            map[wemabod.date] = map[wemabod.date] + 10;
        }
    });
    return { keys: Object.keys(map).join(', '), values: Object.values(map).join(', ') };
}

exports.vehicleMovementByMonth = async (req, res) => {
    let map = {};
