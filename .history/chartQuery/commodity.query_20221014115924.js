...
exports.getProfitByDay = async (req) => {
    ...
    commodity.forEach((e) => {
        //console.log(`${e.dataValues.purchased_date}`.split('-'));
        if (result[`${e.dataValues.purchased_date}`] !== undefined) {
            // result[`${e.dataValues.purchased_date}`].sales_cost = Number.parseInt(`${e.dataValues.sales_cost}`.replace(/,/g, '')) +  result[`${e.dataValues.purchased_date}`].sales_cost;
            // result[`${e.dataValues.purchased_date}`].purchased_cost = Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')) +  result[`${e.dataValues.purchased_date}`].purchased_cost;
            let profit = Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')) - Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')) 

            result[`${e.dataValues.purchased_date}`].profit = 0;
        } else {
            //result[`${e.dataValues.purchased_date}`] = {purchased_cost : Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')), sales_cost : Number.parseInt(`${e.dataValues.sales_cost}`.replace(/,/g, ''))};
            let profit = Number.parseInt(`${e.dataValues.sales_cost}`.replace(/,/g, '')) - Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, ''));
            result[`${e.dataValues.purchased_date}`] = { profit: profit };
        }
    });
    console.log(result);
}