...
exports.getVolumeOfPlasticsWithTonnage = async (req) => {
    const plastics = await req.user.getPlastics({
        attributes: ['volume_of_plastics', [sequelize.fn('COUNT', sequelize.col('volume_of_plastics')), 'total'], 'tonnage'] ,
        group: ['volume_of_plastics', 'tonnage'], order: [[sequelize.fn('COUNT', sequelize.col('volume_of_plastics')), 'DESC']]
    });
    return plastics;
}