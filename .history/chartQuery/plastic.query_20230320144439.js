...
exports.getCrushFlakesWithBarChart = async () => {
    ...
    const weight = await Crushing.findAll({
        ...
        attributes: ['flakes', [sequelize.fn('COUNT', sequelize.col('flakes')), 'total']],
        group: 'flakes', order: [[sequelize.fn('COUNT', sequelize.col('flakes')), 'DESC']],
        raw: true
    });
    weight.forEach((e) => {
...
exports.getCrushCapsWithBarChart = async () => {
    let flakes = [];
    let totals = [];
    const weight = await Crushing.findAll({
        where: {
...
exports.getDynamicTesting = async () => {
    ...
    const weight = await Crushing.findAll({
        ...
        attributes: [[Sequelize.fn('DISTINCT', Sequelize.col('country')) ,'flakes'], [sequelize.fn('SUM', sequelize.col('flakes')), 'total']],
        group: 'flakes', order: [[sequ'flakes')), 'DESC'],
        raw: true
    });
    weight.forEach((e) => {
        flakes.push(e.flakes);
