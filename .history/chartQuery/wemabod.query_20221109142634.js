...
exports.chartPage = async (req, res) => {
    ...
    plastic.forEach(plastics => {
        if (map[plastics.date] === undefined) {
            map[plastics.date] = Number.parseInt(plastics.volume_of_plastics);
        } else {
            map[plastics.date] = map[plastics.initial_content] + Number.parseInt(plastics.volume_of_plastics);
        }
    });
