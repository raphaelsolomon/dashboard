...
exports.getProfitByDay = async (req) => {
    ...
    commodity.forEach((e) => {
        ...
        if(result[`${e.dataValues.purchased_date}`] !== undefined) {
            result[`${e.dataValues.purchased_date}`].sales_cost = Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')) +  result[`${e.dataValues.purchased_date}`].sales_cost;
            result[`${e.dataValues.purchased_date}`].purchased_cost = 
        }else{
