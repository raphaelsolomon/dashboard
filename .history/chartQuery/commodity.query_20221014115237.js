...
exports.getProfitByDay = async (req) => {
    ...
    commodity.forEach((e) => {
        ...
        if(result[`${e.dataValues.purchased_date}`] !== undefined) {
            ...
            result[`${e.dataValues.purchased_date}`] = {'purchased_cost' : Number.parseInt(`${e.dataValues.purchased_cost}`.replace(/,/g, '')), 'sales_cost' : Number.parseInt(`${c.dataValues.sales_cost}`.replace(/,/g, ''))};
        }
    })
}