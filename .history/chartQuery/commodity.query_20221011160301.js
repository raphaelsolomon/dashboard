...
exports.getFrequentPayment = async (req) => {
    ...
    commodity = await Commodity.findAll({
        attributes: ['mode_payment', [sequelize.fn('COUNT', sequelize.col('mode_transaction')), 'total']],
        group: 'mode_payment', order: [[sequelize.fn('COUNT', sequelize.col('mode_transaction')), 'DESC']]
    }) ;
